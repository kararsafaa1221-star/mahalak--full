const fs = require('fs');
const files = ['src/views/Admin/AdminPanel.tsx', 'src/views/Merchant/MerchantApp.tsx'];
...
files.forEach(file => {
  ...
  content = content.replace(/from-indigo-700/g, 'from-[#4D2980]');
  content = content.replace(/to-indigo-905/g, 'to-[#381a66]');
  content = content.replace(/via-indigo-700/g, 'via-[#8040DF]');
...
console.log('Admin and Merchant colors replaced!');
